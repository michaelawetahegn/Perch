...
## Log
- 2026-08-07 — T04–T09: 42 manifest rows, **39 snapshots** via `scripts/harvest.sh`; shared parser
  plumbing is in `data/parse/FeedXml.kt`. **3 exclusions:** `danluu.com` (11.1 MB) and
  `projectzero.google` (13.2 MB) exceed SPEC.md §6's 8 MiB body cap, so the app refuses them live;
  `research.nccgroup.com` publishes no feed — dead.
- 2026-08-07 — T12–T18 done (storage, HTTP, sync, worker). Three rules that outlive them:
  **never Room `@Upsert` for entries** — it resolves on the *primary key*, still 0 on a freshly
  ...
  preserves `isRead`/`readAt`/`isStarred`. **Sanitizing lives in `FeedRepository`.** And
  `observeUnreadCountsByFeed()` is a `GROUP BY` multimap: a **fully-read source is absent from it,
  not 0**.
- 2026-08-07 — T19 done: `ui/theme/*`. Tonal palettes from `#3F6E5A` are **private to Color.kt** —
  screens address `colorScheme` roles, never a tone — and `ArticleType` (serif) is kept out of
  ...
  fallback scheme for the screenshot tests.
...
- 2026-08-07 — T25a: `ArticleLowering`'s input **must** be `HtmlSanitizer` output; the mapper covers
  that allowlist, so `ArticleLoweringCorpusTest` asserts **0 `Unsupported`** (T32 gate 2 allows 2%).
- 2026-08-07 — T25 done: `ui/article/*`. `ArticleBody` is total over the nine blocks with **no
  ...
  **collapses the whole figure on a load error**, so testing one needs a Coil loader that succeeds
  (`Coil.setImageLoader` + a stub `Mapper`). **Residual:** §8's link underline is plain — Compose
  cannot colour or offset one.
...
- 2026-08-07 — T28 done: debug seed data. Seeding goes through **`FeedRepository.add`**, so seeded
  rows are sanitized/deduped like fetched ones and keep their **real** feed URL. The hook is a
  **debug-manifest `ContentProvider`** (release has neither classes nor assets —
  `NoSeederInReleaseTest`), and it seeds only when zero sources exist; under Robolectric construct
  `DebugSeeder` against your own DB, because the provider won't run.
...
- 2026-08-07 — T30 done: `maestro/regression.yaml`, green, driven from Windows. Re-run: copy the yaml
  ...
  Compose node and does **not** reach sheets/dialogs — address those by label; and a Maestro text
...
- 2026-08-07 — T31 done: `fallbackToDestructiveMigration()` is **gone for good**;
  `PerchDatabaseMigrationTest` fails the build on a version bump with no matching migration, a stale
  `app/schemas/N.json`, or the fallback reappearing. `WorkManagerTestInitHelper`'s
  `SynchronousExecutor` misses WorkManager's own task executor, so `cancelUniqueWork` lands
  asynchronously — poll in wall-clock, not virtual time.
- 2026-08-07 — **T32 done.** `acceptance/LiveAcceptanceTest` is in `testDebug`, not `test` (gate 3
  ...
  **prose across blocks**, not its first block. **§8 residuals, cosmetic and not ours:** the LLVM
  feed's own markup omits the spaces around inline `<code>`/`<a>` (`However,until`) — do not
  "repair" it in the lowering; and an inline WordPress-LaTeX image becomes a full-measure block
  mid-sentence. **v0.1 APK (on the phone, debug-signed):**
...
- 2026-08-07 — **U02: losing `~/.perch/perch-release.jks` or `signing.properties` makes every future
  ...
  falls back to debug signing with a warning. Version lives only in `perchVersionCode`/
  `perchVersionName` atop `app/build.gradle.kts`. **`assembleRelease` runs `lintVitalRelease`;
  `assembleDebug` does not.** The manifest drops WorkManager's `WorkManagerInitializer` (which would
  ignore `PerchWorkerFactory`) via `tools:node="merge"` on the shared `InitializationProvider` —
  never `node="remove"`, which takes other libraries' too.
