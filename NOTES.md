...
- 2026-08-07 — T05–T09 done: four parsers + dispatch + `FeedCorpusTest` (39 snapshots; it `check`s
  ≥35 exist, so it cannot go vacuous). Shared plumbing in `data/parse/FeedXml.kt` — reuse it. Date
  floor 2000-01-01 (below → null, caller falls back); >now+24h clamps to now.
...
- 2026-08-07 — T17 done: `data/opml/Opml.kt` + `OpmlRepository`. Import **fetches nothing**: rows
  land never-polled, which T18's worker collects; T27 owns the single refresh. Identity: `feedUrl`.
...
- 2026-08-07 — T23 done: `ui/source/{PastedUrl,AddSourceViewModel,AddSourceSheet}.kt`. **The sheet
  ...
  refuses to guess. The drawer's shut "Add source" row collides by text with the empty state's
  button — tag one.
- 2026-08-07 — T24 done: long-press rename/remove. `NavigationDrawerItem` answers taps only and
  ...
  app bar can hold the same text, so match rows with
...
- 2026-08-07 — T26 done: pull-to-refresh, undo snackbar, `HomeBanner` (offline > selected-source
  ...
  first key), so assert arrivals on list state, never `assertIsDisplayed`. **Pre-existing flake:**
  `PerchNavHostTest`'s article route asserts before `ArticleViewModel`'s DB read — fails under
  full-suite load, passes alone. **T29 residual:** the empty state does not scroll, so it cannot be
  pulled; the overflow's Refresh is the way out of it.
