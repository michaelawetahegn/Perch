...
- 2026-09-21 — **G02b: `./gradlew test` was red on the release variant from G03 to G02b** — `FixtureRasterizer` sat in `src/testDebug` while `src/test` (shared by both variants) used it. It now lives in `src/test/.../support`. **Count both variants** (run with `--continue`); a debug-only green is not the Done-condition. Two more first-run-only flakes that passed alone: `SaveLinkSheetTest > pasting a feed address…`, `PagedFeedTest > a read-state change…`, `PerchNavHostTest > the bottom bar is on every list destination…` (a leaked `ArticleViewModel` feed load, G07b).
