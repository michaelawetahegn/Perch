...
- 2026-08-07 — T19 done: `ui/theme/*`. Tonal palettes from `#3F6E5A` are **private to Color.kt** —
  screens address `colorScheme` roles, never a tone. **Standing grep gate: no `Color(0x`, `N.dp`,
  `N.sp` outside `ui/theme/`.** `PerchTheme(dynamicColor = false)` for screenshots.
...
- 2026-08-07 — T25/T25a done: `ui/article/*`. `ArticleLowering`'s input **must** be `HtmlSanitizer`
  output, so `ArticleLoweringCorpusTest` asserts **0 `Unsupported`**; a source that renders wrong is
  an `ArticleLowering` bug, never a source-specific branch in `ArticleBody`. An image **collapses
  the whole figure on a load error** (right there, wrong in U08's row) — testing one needs a Coil
  loader that succeeds (`Coil.setImageLoader` + a stub `Mapper`).
- 2026-08-07 — T26 done: pull-to-refresh, undo snackbar, `HomeBanner`, `ConnectivityMonitor`
  (`AlwaysOnline` is the `AppContainer` default, so no test needs a shadow network). `PullToRefreshBox`
  ignores a swipe unless its child scrolls, and refresh-prepended rows compose above the viewport —
  assert on list state, never `assertIsDisplayed`. **Residual:** the empty state cannot be pulled.
...
- 2026-08-07 — T29 done: `ui/screenshot/*` → 6 PNGs in `screenshots/`. **Never `captureToImage()`**
  ...
  so draw its `rootView` over the decor view. **Residuals:** zero window insets, so app bar and
  drawer sit flush at y=0; the drawer's Settings row is now below the fold (it scrolls).
- 2026-08-07 — T30 done: `maestro/regression.yaml`, green, driven from Windows (stage it to
  ...
  `testTagsAsResourceId` does **not** reach sheets/dialogs — address those by label, and a Maestro
...
- 2026-08-07 — **U02: losing `~/.perch/perch-release.jks` or `signing.properties` makes every future
  ...
  `PerchWorkerFactory`) via `tools:node="merge"` on `InitializationProvider` — never `node="remove"`.
- 2026-08-07 — **U03 done: DB is version 2** (folders). **Build test databases with
  ...
  **A migration test builds the old DB from `app/schemas/N.json` via
  ...
  `observeUnreadCountsByFolder()` has the same `GROUP BY` trap. **`WorkSchedulerTest` "choosing
  manual cancels the periodic refresh" is flaky in a full-suite run** (the T31 trap) and passes
  alone — re-run before calling it a regression.
...
- 2026-08-07 — **U05 BLOCKED on its gate, not its code.** `data/parse/LeadImage.kt` resolves
  **337/339 (99.4%)** of corpus entries that carry an image, but **only 339 of 1038 carry any image
  markup at all**, so U05's "≥60% of entries" is unreachable — the rung that would close it is
  `og:image`, deferred to U10. Do not chase it with a feed-level `<image>`/`<logo>` fallback: the
  site logo as every thumbnail is the guessed URL §0 forbids, and it games the metric.
  `ThumbnailCorpusTest` gates on *share of available* (≥95%). **Re-gated live at U15 gate 4.**
- 2026-08-07 — **U06 done: folders are in the drawer.** The drawer's scope is now a
  `HomeScope` (All / Folder / Source), and folder is a **second SQL predicate on
  `feeds.folderId`**, never a resolved list of feed ids — a move would invalidate that list.
  `observeListItems`, `unreadIds` (so mark-all-read matches what is on screen) and
  `FeedRepository.refreshFolder` all take the same pair. **Room rejects a `@Query` whose
  parameter it cannot see used**, so an always-true predicate has to still mention `:folderId`.
  Folder headers carry three separate targets (chevron / name / ⋮) and are deliberately
  **not** semantically merged — merging leaves only one of the three reachable.
- 2026-08-07 — `FeedXml.kt`'s `stableGuid` separator must stay `"\u0000"` **escaped**: a raw NUL
  byte in a literal makes git diff the file as binary, so no session can review it.
