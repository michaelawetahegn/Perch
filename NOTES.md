...
## Log
- 2026-09-21 — **PLAN-13 run 1 stopped after G10; G02/G07/G09 re-opened as eight smaller boxes.** All three blocks were avoidable: each task's own §0 section already specified the algorithm, and the sessions improvised. The re-opened boxes carry the anchors below. Do not re-derive them.
- 2026-09-21 — **G02 (now G02a-c): no PDF library. Ever.** §0.3 names `java.util.zip.Inflater` for object streams and SPEC.md §2 lists no PDF dependency — an earlier NOTES entry suggesting PDFBox/iText was wrong and has been deleted. The real defects: `PdfInfo.kt:27-48` takes the *first* `/Title` anywhere in the file (an image XObject on `nist-sp800-63-4`) instead of resolving the last `/Info N 0 R`; `extractLiteralString` (`:57-98`) has no `FE FF` BOM check, only `decodeHex` (`:131`) does; `extractFromXmp` (`:141-150`) takes the last `<dc:title>` not the first; `isBoilerplate` (`:196-205`) lacks `PowerPoint Presentation`, `Slide 1` and title==filename. **And the box was unachievable as written:** the `creationDate` case asserted a `published` for `encrypted-empty-user-password`, which §0.3 requires to yield null — G02a fixes that first.
- 2026-09-21 — **G07 (now G07a-c): gestures work in this repo.** The blocked session reported Compose gesture APIs unavailable; that is false. `ImageViewerTest.kt:86-92` uses `pinch(start0=, end0=, start1=, end1=)` and `doubleClick()` at `:104`/`:109`, reused at `LiveAcceptanceTest.kt:1458`. Its failures were arities/types: bare `pinch()`, bare `performScrollToNode()`, and `.filter { lambda }` where it takes a `SemanticsMatcher`. Two real gaps left behind: the page item (`DocumentBody.kt:133-175`) draws grey placeholders, never a bitmap; and `pageUnderCentre` at `:229-233` is a `// TODO` stub returning null, while the real one at `PagePosition.kt:9-25` takes a `LayoutItem` that `LazyListItemInfo` does not implement — adapt it, do not write a third.
- 2026-09-21 — **G09 (now G09a-b): the feature landed, the tests did not.** `fc3aa2c`/`0eaaf32` shipped the manifest filters, `incomingFrom`, `intake`, `saveDocument` and the shared `storeDocument` sink; do not rewrite them. Missing: `IncomingShareTest` (does not exist), document cases in `SavedLinkRepositoryTest` (the one at `:211` exercises `saveLink`, not `saveDocument`) and any `intake`/`submitDocument` case in `SaveLinkViewModelTest`. `SaveLinkSheet.kt:95` also passes `displayName = null` — the `OpenableColumns.DISPLAY_NAME` lookup was never built.
...
- 2026-09-21 — **G02b: `./gradlew test` was red on the release variant from G03 to G02b** — `FixtureRasterizer` sat in `src/testDebug` while `src/test` (shared by both variants) used it. It now lives in `src/test/.../support`. **Count both variants** (run with `--continue`); a debug-only green is not the Done-condition. Two more first-run-only flakes that passed alone: `SaveLinkSheetTest > pasting a feed address…`, `PagedFeedTest > a read-state change…`.
- 2026-09-08 — **Two full-suite-only flakes; green alone and on a re-run, so re-run before diagnosing.**
  `WorkSchedulerTest > choosing manual cancels…` (3 of 5 by D15) waits on WorkManager's own executor, which
